...
<template>
  <div class="dark:bg-gray-800 p-10 rounded-md">
    <div class="mb-5 flex space-x-2 w-10 text-center">
      <input
        class="flex-grow justify-center dark:bg-gray-700 dark:text-white px-3 py-2 rounded-l-md transition-colors duration-200 ease-in-out hover:bg-gray-600"
    ...
    <aside
      class="absolute top-20 left-0 max-h-96 w-75 z-10 flex flex-col items-start bg-gray-900 border rounded-md shadow-md mt-1"
      role="menu"
      v-if="movieStore.autocompleteResults?.length > 0">
      <ul class="flex flex-col w-full overflow-y-scroll h-auto">
        <li
